...
def cudnnrnn(rnn_type, inputs, length, hidden_size, layer_num=1, dropout_keep_prob=1):
    outputs = None
    states = None

    if rnn_type.endswith('lstm'):
        lstm = tc.cudnn_rnn.CudnnLSTM(layer_num, hidden_size, direction='bidirectional',dropout=1-dropout_keep_prob)
        outputs, states = lstm(tf.transpose(inputs, (1, 0, 2)))
    ...
    if rnn_type.endswith('gru'):
        gru = tc.cudnn_rnn.CudnnGRU(layer_num, hidden_size, direction='bidirectional',dropout=1-dropout_keep_prob)
        outputs, states = gru(tf.transpose(inputs, (1, 0, 2)))
        outputs = tf.transpose(outputs, (1, 0, 2))
    ...
    states_fw = tf.squeeze(states[0])
    states_bw = tf.squeeze(states[1])
    states = tf.concat([states_fw,states_bw],axis=-1)
...
def encoder_block(inputs, num_blocks, num_conv_layers, kernel_size, mask = None,
...
def residual_block(inputs, num_blocks = 1, num_conv_layers = 0, kernel_size = None , mask = None,
                   num_filters = 128, input_projection = False, num_heads = 8,
                   seq_len = None, scope = "res_block", is_training = True,
                   reuse = None, bias = True, dropout = 0.0):
...
def my_self_attention(p,p_len = None):
    '''

    :param p:  batch_size,max_p_len,dim
    :param p_len: batch_size
    :return: batch_size,max_p_len,dim*4
    '''
    q = p
    k = p
    v = p
    logits = tf.matmul(q, k, transpose_b=True)#batch,max_p,max_p
    if p_len != None:
        mask = tf.cast(tf.sequence_mask(p_len), tf.float32)#batch,max_p
        mask = tf.expand_dims(mask,axis=1)#batch,1,max_p
        logits =exp_mask(logits,mask)
    weights = tf.nn.softmax(logits)
    o = tf.matmul(weights,v)#batch_size,max_p_len,dim
    output = tf.concat([v,o,v*o,v-o],axis=-1)#batch_size,max_p_len,dim*4
    return o,output

def my_self_attention_word_level(p,p_len = None):

    q = p# 1,batch*max_p,dim
    k = p# 1,batch*max_p,dim
    v = p# 1,batch*max_p,dim
    logits = tf.matmul(q, k, transpose_b=True)  # 1,batch*max_p,batch*max_p

    if p_len !=None:
        mask = tf.cast(tf.sequence_mask(p_len), tf.float32)  # batch,max_p
        mask = tf.reshape(mask, [1, 1, -1])  # 1,1,batch*max_p
        logits = exp_mask(logits, mask)  # 1,batch*max_p,batch*max_p

    weights = tf.nn.softmax(logits)  # 1,batch*max_p,batch*max_p
    o = tf.matmul(weights, v)  # 1,batch*max_p,dim
    p_sa = p + o  # 1,batch*max_p,dim
    return p_sa

def my_self_attention_word_level_batch(p,p_len = None,batch_size = None):

    q = p# #8,100*max_p,dim
    k = p# #8,100*max_p,dim
    v = p# #8,100*max_p,dim
    logits = tf.matmul(q, k, transpose_b=True)  # 8,100*max_p,100*max_p

    if p_len !=None:
        mask = tf.cast(tf.sequence_mask(p_len), tf.float32)  # 8*100,max_p
        mask = tf.reshape(mask, [batch_size, 1, -1])  # 8,1,100*max_p
        logits = exp_mask(logits, mask)  # 8,100*max_p,100*max_p

    weights = tf.nn.softmax(logits)  # 8,100*max_p,100*max_p
    o = tf.matmul(weights, v)  # 8,100*max_p,dim
    p_sa = p + o  # 8,100*max_p,dim

    return p_sa

def my_co_attention(passage,question,question_len = None):
    '''

    :param p:  batch_size,max_p_len,dim
    :param q:  batch_size,max_q_len,dim
    :param p_len: batch_size
    :return: batch_size,max_p_len,dim*4
    '''
    q = passage
    ...
    v = question
    k_len = question_len
    logits = tf.matmul(q, k, transpose_b=True)#batch,max_p,max_q
    if k_len != None:
        mask = tf.cast(tf.sequence_mask(k_len), tf.float32)#batch,max_q
        mask = tf.expand_dims(mask,axis=1)#batch,1,max_q
        logits =exp_mask(logits,mask)#batch,max_p,max_q
    weights = tf.nn.softmax(logits)
    o = tf.matmul(weights,v)#batch_size,max_p_len,dim
    return o

def my_co_attention_pool(p,q,p_len=None):
    '''

    :param p:  batch_size,max_p_len,dim
    :param q:  batch_size,dim
    :param p_len: batch_size
    :return: batch_size,dim
    '''
    q = tf.expand_dims(q,axis=1)#batch_size,1,dim
    logits = tf.matmul(p, q, transpose_b=True)#batch_size,max_p_len,1
    if p_len != None:
        mask = tf.cast(tf.sequence_mask(p_len), tf.float32)  # batch,max_p
        mask = tf.expand_dims(mask, axis=-1)# batch,max_p,1
        logits = exp_mask(logits, mask)# batch,max_p,1
    weights = tf.nn.softmax(logits,axis=1)# batch,max_p,1
    output = tf.reduce_sum(p * weights, axis=1)#batch_size,dim
    return output

def my_self_attention_pool(pooling_vectors, ref_vector, hidden_size, p_len = None,scope=None):
    """
        Applies attend pooling to a set of vectors according to a reference vector.
        Args:
            pooling_vectors: the vectors to pool                   #batch_size,max_p_len,dim
            ref_vector: the reference vector                       #1,dim
            hidden_size: the hidden size for attention function    #dim
            p_len:                                                 #batch_size
            scope: score name
        Returns:
            the pooled vector                                      #batch_size,dim
        """
    with tf.variable_scope(scope or 'attend_pooling'):

        U = tf.tanh(tf.contrib.layers.fully_connected(pooling_vectors, num_outputs=hidden_size,
                                              activation_fn=None, biases_initializer=None)
                    + tf.contrib.layers.fully_connected(tf.expand_dims(ref_vector, 1),
                                                num_outputs=hidden_size,
                                                activation_fn=None))

        logits = tf.contrib.layers.fully_connected(U, num_outputs=1, activation_fn=None) # batch,max_p,1
        ...
        output = tf.reduce_sum(pooling_vectors * weights, axis=1)#batch_size,dim
    return output
