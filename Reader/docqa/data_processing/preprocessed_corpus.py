...
class PreprocessedData(TrainingData):
    ...
    def preprocess(self, epoch=1, chunk_size=500):
        print("Loading data...")
        train_questions = 111
        dev_questions = 111
        ...
        if self.preprocesser:
            ...
            #todo 2
            print("Preprocessing with %d epoch..." % epoch)
            out = []
            for name, questions in [("verified", verified_questions),
                ...
                if questions is None:
                    out.append(None)
                    continue
                data = self.preprocesser.init_train_data(name,epoch)
                print (name,len(data.data))
                self.preprocesser.finalize_chunk(data)
                out.append(data)
