...
class EvaluatorRunner(object):
    ...
    def run_evaluators(self, sess: tf.Session, dataset: Dataset, name, n_sample=None, feed_dict=None) -> Evaluation:
        ...
        tmp_dict = self.tensors_needed[0]
        
        if 'start_logits' in tmp_dict.keys():

            #tensor_span_scores = tmp_dict['span_scores']
            tensor_start_logits = tmp_dict['start_logits']
            tensor_end_logits = tmp_dict['end_logits']

            #my_span_scores = tensors[tensor_span_scores]
            start_probs = tensors[tensor_start_logits]
            end_probs = tensors[tensor_end_logits]

            #print (len(start_probs))
            #print (len(end_probs))
            #print (len(my_span_scores))
            tmp_time = datetime.now().strftime("%H%M%S")
            #with open('probs/start_probs_'+tmp_time+'.pkl','wb') as fout:
            #    pickle.dump(start_probs,fout)
            #with open('probs/end_probs_'+tmp_time+'.pkl','wb') as fout:
            #    pickle.dump(end_probs,fout)

            with open('probs/start_probs.pkl','wb') as fout:
                pickle.dump(start_probs,fout)
            with open('probs/end_probs.pkl','wb') as fout:
                pickle.dump(end_probs,fout)


            #with open('end_probs.pkl','wb') as fout:
            #    pickle.dump(end_probs,fout)        
...
class AysncEvaluatorRunner(object):
    ...
    def set_input(self, prediction: Prediction):
        ...
        #tensor_start_logits = tmp_dict['start_logits']
        #tensor_end_logits = tmp_dict['end_logits']
        #print (tensor_span_scores)
        #print (tensor_start_logits)
        #print (tensor_end_logits)
    ...
    def run_evaluators(self, sess: tf.Session, dataset, name, n_sample, feed_dict) -> Evaluation:
        ...
        for _ in tqdm(range(n_batches), total=n_batches, desc=name, ncols=80):
            ...
            #tmp_end_probs = list(output[0])  # (200, 2350)
            #end_probs+=tmp_end_probs
            #tmp_start_probs = list(output[3])  # (200, 2350)
            #start_probs += tmp_start_probs
        th.join()
        tmp_dict = self.tensors_needed[0]
        #tensor_span_scores = tmp_dict['span_scores']
        if 'start_logits' in tmp_dict.keys():
            ...
            with open('start_probs.pkl','wb') as fout:
                pickle.dump(start_probs,fout)

            with open('end_probs.pkl','wb') as fout:
                pickle.dump(end_probs,fout)

            #with open('my_span_scores.pkl','wb') as fout:
                #pickle.dump(my_span_scores,fout)
