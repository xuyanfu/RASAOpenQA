...
class DocumentAndQuestionEncoder(Configurable):
    ...
    def init(self, input_spec: ParagraphAndQuestionSpec, len_op: bool,
        ...
        self.cw_ebd = tf.nn.embedding_lookup(self.cw_mat, self.cw)
    ...
    def get_placeholders(self):
      ...
      '''
      return [x for x in [self.question_len, self.question_words, self.question_chars, self.question_features,self.context_len, self.context_words, self.context_chars, self.context_features,self.question_word_len, self.context_word_len,self.cw,self.cw_q] if x is not None] + self.answer_encoder.get_placeholders()
    ...
    def encode(self, batch: List[ContextAndQuestion], is_train: bool):
        ...
        list_cw = np.zeros((batch_size, context_word_dim),dtype='int32')
        list_cw_q = np.zeros((batch_size, ques_word_dim),dtype='int32')
        for doc_ix, doc in enumerate(batch):
            q_words = doc.question
            p_words = doc.context
            tmp_q = ' '.join(q_words)
            tmp_q = tmp_q.lower()
            tmp_p = ' '.join(p_words)
            ...
            for w_ix, word in enumerate(p_words):
                word = word.lower()
                if word in tmp_q:
                    list_cw[doc_ix][w_ix] = 1
            for w_ix, word in enumerate(q_words):
                word = word.lower()
                if word in tmp_p:
                    list_cw_q[doc_ix][w_ix] = 1

        feed_dict[self.cw] = list_cw
        feed_dict[self.cw_q] = list_cw_q
        #print (list_cw_q)
        #print (list_cw_q.shape)
        #print (question_words.shape)
        return feed_dict
