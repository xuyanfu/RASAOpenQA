...
def main():
    ...
    for item in train_set:
        tmp_q = ' '.join(item.question)
        tmp_list_a = item.answer_text
        tmp_list_p = []
        tmp_q_id = item.question_id
        for para in item.paragraphs:
            tmp_list_p.append(' '.join(para.text))

        list_train_q.append(tmp_q)
        list_list_train_p.append(tmp_list_p)
        list_list_train_a.append(tmp_list_a)
        list_train_q_id.append(tmp_q_id)



    with open('list_train_q_id.pkl','wb') as fout:
        pickle.dump(list_train_q_id,fout)

    with open('list_train_q.pkl','wb') as fout:
        pickle.dump(list_train_q,fout)

    with open('list_train_a.pkl','wb') as fout:
        pickle.dump(list_list_train_a,fout)

    with open('list_train_p.pkl','wb') as fout:
        pickle.dump(list_list_train_p,fout)

    print('len(list_train_q_id):', len(list_train_q_id))
